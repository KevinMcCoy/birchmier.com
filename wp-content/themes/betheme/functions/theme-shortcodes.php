		...
		$output = '<div class="zoom_box">';
			if( $link ) $output .= '<a href="'. $link .'" '. $target .'>';
				...
				$output .= '<div class="photo">';
					$output .= '<img class="scale-with-grid" src="'. $image .'" alt="" '. $width .' '. $height .'/>';
				$output .= '</div>';
		...
		$output = '<div class="story_box '. $style .'">';
			if( $animate ) $output .= '<div class="animate" data-anim-type="'. $animate .'">';
				if( $link ) $output .= '<a href="'. $link .'" '. $target .'>';
					...
					$output .= '<div class="photo_wrapper">';
						$output .= '<img class="scale-with-grid" src="'. $image .'" alt="'. strip_tags( $title ) .'" '. $width .' '. $height .'/>';
					$output .= '</div>';
		...
		$output = '<div class="promo_box '. $border .'">';
			if( $animate ) $output .= '<div class="animate" data-anim-type="'. $animate .'">';
				$output .= '<div class="promo_box_wrapper promo_box_'. $position .'">';
					...
					$output .= '<div class="photo_wrapper">';
						if( $image ) $output .= '<img class="scale-with-grid" src="'. $image .'" alt="'. $title .'" '. $width .' '. $height .'>';
					$output .= '</div>';
			...
			if( $animate ) $output .= '<div class="animate" data-anim-type="'. $animate .'">';
					...
					if( $link ) $output .= '<a href="'. $link .'" '. $target .'>';
						$output .= '<div class="image">';
							$output .= '<img src="'. $image .'" class="scale-with-grid" alt="'. $title .'" '. $width .' '. $height .'>';
	...
	function sc_chart( $attr, $content = null )
	{
		...
		$output = '<div class="chart_box">';
			$output .= '<div class="chart" data-percent="'. intval($percent) .'" data-color="'.  $color .'">';
				if( $image ){
					$output .= '<div class="image"><img class="scale-with-grid" src="'. $image .'" alt="'. $percent .'" '. $width .' '. $height .' /></div>';
				} elseif( $icon ){
...
if( ! function_exists( 'sc_image' ) )
{
	function sc_image( $attr, $content = null )
	{
		...
		$attachmentID = mfn_get_attachment_id_url( $src );
		if( ! $width ) 	$width 	= mfn_get_attachment_dimensions( $attachmentID, 'width' );
		if( ! $height ) $height = mfn_get_attachment_dimensions( $attachmentID, 'height' );
...
if( ! function_exists( 'sc_hover_box' ) )
{
	function sc_hover_box( $attr, $content = null )
	{
		...
		$output = '<div class="hover_box" ontouchstart="this.classList.toggle(\'hover\');">';
			if( $link ) $output .= '<a href="'. $link .'" '. $target .'>';
				$output .= '<div class="hover_box_wrapper">';
					$output .= '<img class="visible_photo scale-with-grid" src="'. $image .'" alt="" '. $width .' '. $height .'/>';
					$output .= '<img class="hidden_photo scale-with-grid" src="'. $image_hover .'" alt="" '. $width .' '. $height .'/>';
				$output .= '</div>';
			if( $link ) $output .= '</a>';
		$output .= '</div>'."\n";
		...
		if( $animate ) $output .= '<div class="animate" data-anim-type="'. $animate .'">';
			$output .= '<div class="icon_box icon_position_'. $icon_position .' '. $border .'">';
				if( $link ) $output .= '<a class="'. $class .'" href="'. $link .'" '. $target .' '. $rel .'>';
					...
					if( $image ){
						$output .= '<div class="image_wrapper">';
							$output .= '<img src="'. $image .'" alt="'. $title .'" class="scale-with-grid" '. $width .' '. $height .'/>';
						$output .= '</div>';
					} else {
						$output .= '<div class="icon_wrapper">';
